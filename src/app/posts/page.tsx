...
export const metadata: Metadata = {
  title: "質問一覧 | オルマ",
  description:
    "数学の質問投稿一覧。高校数学〜大学受験の問題を検索し、解答や解説で理解を深められます。",
  openGraph: {
    title: "質問一覧 | オルマ",
    description:
      "数学の質問投稿一覧。高校数学〜大学受験の問題を検索し、解答や解説で理解を深められます。",
...
export default async function PostsIndex({
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <header className="rounded-[24px] border border-slate-300/80 bg-white/95 p-5 sm:p-6 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <div className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Q&A</div>
            <h1 className="text-xl sm:text-2xl font-semibold">Q&A 一覧</h1>
            <p className="text-xs sm:text-sm text-slate-600">質問・回答・議論の一覧</p>
          </div>
          <div className="flex flex-wrap gap-2 sm:flex-row sm:items-center">
            <Link href="/posts/new" className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-300 bg-slate-900 px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm hover:bg-slate-800 transition w-full sm:w-auto">
              新規質問
            </Link>
            <Link href="/posts/unanswered" className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-3 py-2 text-xs sm:text-sm text-slate-700 hover:bg-slate-50 transition w-full sm:w-auto">
              未回答
            </Link>
            <Link href={`/posts?${new URLSearchParams({ q: qRaw, level, solved: "no", page: "1" }).toString()}`} className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-3 py-2 text-xs sm:text-sm text-slate-700 hover:bg-slate-50 transition w-full sm:w-auto">
              未回答のみ
            </Link>
          </div>
        </div>
      ...
      {serverError ? (
        <div className="rounded-[20px] border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] sm:text-sm text-rose-700">
          {serverError}
        </div>
      ) : null}

      {/* 検索＆フィルタ（同ページ） */}
      <form className="rounded-[20px] border border-slate-300/80 bg-white/95 p-4 sm:p-5 shadow-sm" aria-label="Q&A検索フォーム" role="search" method="get">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
          <input
            name="q"
            defaultValue={qRaw}
            className="border border-slate-200 px-3 py-2 rounded-lg col-span-2 text-[12px] sm:text-sm focus:outline-none focus:ring-2 focus:ring-slate-200"
            placeholder="キーワード（タイトル・本文・タグ／OR）"
            aria-label="Q&A検索"
          />
          <select name="level" defaultValue={level} className="border border-slate-200 px-2 py-2 rounded-lg text-[12px] sm:text-sm">
            <option value="">難易度すべて</option>
            <option value="HS">高校</option>
            <option value="UG">大学基礎</option>
            <option value="ADV">専門</option>
          </select>
          <select name="solved" defaultValue={solvedFilter} className="border border-slate-200 px-2 py-2 rounded-lg text-[12px] sm:text-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button className="inline-flex items-center justify-center rounded-full border border-slate-300 bg-slate-900 px-4 py-2 text-[12px] sm:text-sm font-semibold text-white shadow-sm hover:bg-slate-800" aria-label="検索を実行">
            検索
          </button>
          {qRaw && (
            <Link href="/posts" className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-4 py-2 text-[12px] sm:text-sm text-slate-700 hover:bg-slate-50">
              クリア
            </Link>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 text-[11px] sm:text-xs">
        {forceOpen ? (
          <Link href={`/posts?${qsBase}`} className="border border-slate-200 rounded-full px-3 py-1 bg-white hover:bg-slate-50">
            タグを折りたたむ
          </Link>
        ) : (
          <Link href={`/posts?${qsBase}&tags=open`} className="border border-slate-200 rounded-full px-3 py-1 bg-white hover:bg-slate-50">
            タグを一括展開
          </Link>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 text-[11px] sm:text-xs">
        {sort === "top" ? (
          <Link
            ...
            className="border border-slate-200 rounded-full px-3 py-1 bg-white hover:bg-slate-50"
          >
            新着
          </Link>
        ) : (
          <Link
            ...
            className="border border-amber-200 rounded-full px-3 py-1 bg-amber-50 text-amber-700 hover:bg-amber-100"
          >
            人気 🔥
          </Link>
        )}
        <span className="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[10px] sm:text-xs">
        ...
        {savedOnly ? (
          <Link href={`/posts?${new URLSearchParams({ q: qRaw, level, solved: solvedFilter, page: String(page) }).toString()}`} className="border border-slate-200 rounded-full px-3 py-1 bg-white hover:bg-slate-50">
            すべて
          </Link>
        ) : (
          <Link href={`/posts?${new URLSearchParams({ q: qRaw, level, solved: solvedFilter, page: String(page), saved: "1" }).toString()}`} className="border border-slate-200 rounded-full px-3 py-1 bg-white hover:bg-slate-50">
            保存のみ
          </Link>
        )}
        {hasFilters && (
          <Link href="/posts" className="border border-slate-200 rounded-full px-3 py-1 bg-white hover:bg-slate-50" aria-label="条件をクリア" role="button">
            条件をクリア
          </Link>
        )}
        {hasFilters && (
          <span className="px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-700" aria-label="フィルタ適用中">
            フィルタ中
          </span>
        )}
      </div>

      <div className="text-[11px] sm:text-xs text-slate-500 flex flex-wrap gap-x-3 gap-y-1" role="status" aria-live="polite" aria-atomic="true">
      ...
      {pageRows.length > 0 && (
        <div className="rounded-[20px] border border-slate-300/80 bg-white/95 p-3 sm:p-4 shadow-sm">
          <div className="text-[11px] sm:text-xs text-slate-500">人気タグ（検索ショートカット）</div>
          <div className="mt-2 flex flex-wrap gap-2">
            {collectTopTags(pageRows, 10).map((tag) => (
              <Link
                key={tag}
                href={`/posts?${new URLSearchParams({ q: tag, level, solved: solvedFilter, page: "1" }).toString()}`}
                className="text-[11px] sm:text-xs px-3 py-1 rounded-full border border-slate-200 bg-white hover:bg-slate-50"
              >
                #{tag}
              </Link>
            ))}
      ...
      <ul id="posts-list" className={`grid gap-3 sm:gap-4 md:gap-5 ${savedOnly ? "saved-only" : ""}`} data-saved-container>
        ...
        {pageRows.map((p: any) => {
          ...
          return (
            <li key={p.id} className="rounded-[20px] border border-slate-300/80 bg-white/95 p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition" data-saved-item data-saved-id={String(p.id)}>
              <div className="flex items-start gap-3">
                <div className="flex flex-col gap-1">
                  <span className={`text-[11px] sm:text-xs px-2.5 py-1 rounded-full mt-1 ${st.solved ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                    {st.solved ? "解決" : "未解決"}
                  </span>
                  {st.answers_count === 0 && (
                    <span className="text-[11px] sm:text-xs px-2.5 py-1 rounded-full bg-red-100 text-red-700">
                      未回答
                    </span>
                  )}
                </div>
                <div className="flex-1">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <Link href={`/posts/${p.id}`} className="font-medium text-[15px] sm:text-base hover:underline">
                      ...
                      <span className="saved-badge ml-2 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[10px]">保存</span>
                      {sort === "top" && (
                        <span className="ml-2 px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 text-[10px]">人気</span>
                      )}
                    </Link>
                  ...
                  <div className="text-[11px] sm:text-sm text-slate-500 mt-1.5 flex flex-wrap gap-x-2 gap-y-1">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5 text-[11px] sm:text-xs">
                      {p.tags.slice(0, 3).map((tag: string) => (
                        <Link
                          ...
                          className="px-2.5 py-1 bg-slate-100 rounded-full hover:bg-slate-200"
                        >
                          {tag}
                        </Link>
                      ))}
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-[11px] sm:text-xs">
                    <Link href={`/posts/${p.id}#comments`} className="text-slate-700 hover:underline px-3 py-1 rounded-full bg-slate-50 border border-slate-200">コメントへ</Link>
                  </div>
                </div>
              </div>
            </li>
        ...
        {pageRows.length === 0 && (
          <li className="rounded-[20px] border border-slate-300/80 bg-white/95 p-4 text-[11px] sm:text-sm text-slate-500 space-y-2" role="status" aria-live="polite">
            <div>該当する質問がありません。</div>
            <Link href="/posts/new" className="text-blue-700 hover:underline">新規質問を作成</Link>
          </li>
        )}
        <li className="rounded-[20px] border border-slate-300/80 bg-white/95 p-4 text-[11px] sm:text-sm text-slate-500 hidden" data-saved-empty>保存済みの質問がありません。</li>
      </ul>
