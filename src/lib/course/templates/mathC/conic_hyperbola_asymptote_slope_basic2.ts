...
function buildTemplate(id: string, title: string): QuestionTemplate {
  return {
    ...
    generate() {
      ...
      const statement = `測定で得た反射鏡の断面を表す双曲線 $\\frac{x^2}{${params.a ** 2}}-\\frac{y^2}{${params.b ** 2}}=1$ の測定された設計上の漸近線の傾きを求めよ。`;
      return {
        templateId: id,
        statement,
        answerKind: "numeric",
...
export const conicHyperbolaAsymptoteSlopeExtraTemplates: QuestionTemplate[] = Array.from({ length: 6 }, (_, i) =>
  buildTemplate(`conic_hyperbola_asymptote_slope_basic2_${i + 1}`, `測定された設計上の漸近線の傾き ${i + 1}`)
);
