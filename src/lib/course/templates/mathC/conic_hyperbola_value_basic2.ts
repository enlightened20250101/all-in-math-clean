...
function buildTemplate(id: string, title: string): QuestionTemplate {
  return {
    ...
    generate() {
      ...
      const statement = `双曲線 $\\frac{x^2}{${params.a ** 2}}-\\frac{y^2}{${params.b ** 2}}=1$ 上で $x=${params.x}$ のときの $y$ の値を求めよ（正の値）。`;
      return {
        templateId: id,
        statement,
        answerKind: "numeric",
...
export const conicHyperbolaValueExtraTemplates: QuestionTemplate[] = Array.from({ length: 6 }, (_, i) =>
  buildTemplate(`conic_hyperbola_value_basic2_${i + 1}`, `双曲線の値 ${i + 1}`)
);
