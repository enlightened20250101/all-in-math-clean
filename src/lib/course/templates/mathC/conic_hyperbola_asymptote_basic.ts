...
function buildTemplate(id: string, title: string): QuestionTemplate {
  return {
    ...
    generate() {
      ...
      const statement = `測定で得た反射鏡の断面を表す双曲線 $\\frac{x^2}{${c.a2}}-\\frac{y^2}{${c.b2}}=1$ の測定された設計上の漸近線 $y=mx$ のうち、$m>0$ を求めよ。`;
      return {
        templateId: id,
        statement,
        answerKind: "choice",
    ...
    explain(params) {
      ...
      return `
### この問題の解説
測定された設計上の漸近線は $y=\\pm\\frac{b}{a}x$ なので ${c.explain} です。
答えは **${c.answer}** です。
`;
    },
  };
...
export const conicHyperbolaAsymptoteTemplates: QuestionTemplate[] = Array.from({ length: 6 }, (_, i) =>
  buildTemplate(`conic_hyperbola_asymptote_basic_${i + 1}`, `測定で得た反射鏡の断面を表す双曲線の測定された設計上の漸近線 ${i + 1}`)
);

const extraHyperbolaAsymptoteTemplates: QuestionTemplate[] = Array.from({ length: 44 }, (_, i) =>
  buildTemplate(`conic_hyperbola_asymptote_basic_${i + 7}`, `測定で得た反射鏡の断面を表す双曲線の測定された設計上の漸近線 追加${i + 1}`)
);
