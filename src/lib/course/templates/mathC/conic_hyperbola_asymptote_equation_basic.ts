...
function buildTemplate(id: string, title: string): QuestionTemplate {
  return {
    ...
    generate() {
      ...
      const statement = `双曲線 $\\frac{x^2}{${params.a ** 2}}-\\frac{y^2}{${params.b ** 2}}=1$ の漸近線 $y=mx$ の係数 $m$ を求めよ。`;
      return {
        templateId: id,
        statement,
        answerKind: "numeric",
...
export const conicHyperbolaAsymptoteEquationTemplates: QuestionTemplate[] = Array.from({ length: 6 }, (_, i) =>
  buildTemplate(`conic_hyperbola_asymptote_equation_basic_${i + 1}`, `漸近線 ${i + 1}`)
);


const extraAsymptoteEquationTemplates: QuestionTemplate[] = Array.from({ length: 38 }, (_, i) =>
  buildTemplate(`conic_hyperbola_asymptote_equation_basic_${i + 7}`, `漸近線 追加${i + 1}`)
);
