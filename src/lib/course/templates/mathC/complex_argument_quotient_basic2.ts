...
function buildTemplate(id: string, title: string): QuestionTemplate {
  return {
    ...
    generate() {
      ...
      const statement = `複素数 $z_1,z_2$ の点の偏角がそれぞれ ${params.a}^\\circ, ${params.b}^\\circ$ のとき、$\\frac{z_1}{z_2}$ の点の偏角を $0^\\circ\\le\\theta<360^\\circ$ で答えよ。`;
      return {
        templateId: id,
        statement,
        answerKind: "numeric",
...
export const complexArgumentQuotientExtraTemplates: QuestionTemplate[] = Array.from({ length: 6 }, (_, i) =>
  buildTemplate(`complex_argument_quotient_basic2_${i + 1}`, `点の偏角の差 ${i + 1}`)
);
