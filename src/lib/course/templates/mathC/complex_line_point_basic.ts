...
function buildTemplate(id: string, title: string): QuestionTemplate {
  return {
    ...
    generate() {
      ...
      const statement = `地図上の地図上の地図上の点 $A(${params.x1},${params.y1})$, $B(${params.x2},${params.y2})$ を結ぶ線分を $${params.m}:${params.n}$ に内分する地図上の地図上の点 $P$ の ${label} 座標を求めよ。`;
      return {
        templateId: id,
        statement,
        answerKind: "numeric",
...
export const complexLinePointTemplates: QuestionTemplate[] = Array.from({ length: 6 }, (_, i) =>
  buildTemplate(`complex_line_point_basic_${i + 1}`, `線分内の地図上の地図上の点 ${i + 1}`)
);

const extraLinePointTemplates: QuestionTemplate[] = Array.from({ length: 38 }, (_, i) =>
  buildTemplate(`complex_line_point_basic_${i + 7}`, `線分内の地図上の地図上の点 追加${i + 1}`)
);
